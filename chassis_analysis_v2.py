"""
Truck chassis structural analysis desktop app V2.

This chassis-only version focuses on truck frame/rail strength checks using
first-pass strength of materials calculations. It is useful for ASM project
study work and presentation graphs, but it is not a replacement for full FEA
validation in ANSYS, HyperMesh, Abaqus, or similar tools.
"""
# python chassis_analysis_v2.py
...
from tkinter import BOTH, BooleanVar, Canvas, END, LEFT, RIGHT, StringVar, Tk, Toplevel, filedialog, messagebox, ttk
...
class ChassisApp:
    def __init__(self, root: Tk):
        self.root = root
        self.root.title("Truck Chassis Structural Analysis V2")
    ...
    def build_ui(self):
        ...
        style.configure("Metric.TLabel", font=("Segoe UI", metric_size, "bold"))
        ...
        ttk.Label(header, text="Truck Chassis Structural Analysis V2", style="Title.TLabel").pack(anchor="w")
        ttk.Label(
            header,
            text="Chassis rail, cross-member, and fitting-zone stress, strain, deformation, factor of safety, material comparison, and PDF report.",
            style="Sub.TLabel",
        ).pack(anchor="w")
    ...
    def build_inputs(self, parent):
        ...
        file_box = ttk.LabelFrame(setup_tab, text="Chassis / CAD Input", padding=10)
        ...
        comp_box = ttk.LabelFrame(setup_tab, text="Chassis Specification Table", padding=10)
        ...
        mat_box = ttk.LabelFrame(setup_tab, text="Material Used For Chassis", padding=10)
        ...
        load_box = ttk.LabelFrame(load_tab, text="Load Input Table", padding=10)
        ...
        bc_box = ttk.LabelFrame(load_tab, text="Boundary Condition", padding=10)
        bc_box.pack(fill="x", pady=(0, 8))
        ttk.Combobox(bc_box, textvariable=self.boundary, values=BOUNDARIES, state="readonly").pack(fill="x")

        cfd_box = ttk.LabelFrame(road_tab, text="Optional V2 Road/Air Load", padding=10)
    ...
    def upload_part(self):
        ...
        if "body" in name or "cab" in name or "fitting" in name:
            self.component_name.set("Body Mount Bracket Zone")
        elif "cross" in name or "member" in name:
            self.component_name.set("Cross Member")
        elif "suspension" in name or "bracket" in name or "mount" in name:
            self.component_name.set("Suspension Bracket Zone")
        elif "box" in name or "boxed" in name:
            self.component_name.set("Truck Chassis Box Rail")
        elif "4x2" in name or "four" in name:
            self.component_name.set("4x2 Truck Main Chassis Rail")
        elif "rail" in name or "chassis" in name or "frame" in name or "beam" in name:
            self.component_name.set("Truck Chassis C-Section Rail")
        self.load_component_defaults()
    ...
    def open_graph_window(self):
        ...
        graph_window.title("Chassis Analysis Graphs")
        graph_window.geometry("1500x850+120+80")
        graph_window.minsize(1100, 700)

        toolbar = ttk.Frame(graph_window, padding=8)
        toolbar.pack(fill="x")
        ttk.Label(toolbar, text="Graph View - Stress, Strain, Deformation, and FOS", style="Title.TLabel").pack(side=LEFT)
    ...
    def save_figure_from_window(self, figure):
        path = filedialog.asksaveasfilename(
            title="Save graph window image",
            defaultextension=".png",
            initialfile="chassis_analysis_graph_window.png",
    ...
    def export_graphs(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            defaultextension=".png",
            initialfile="chassis_analysis_v2_graphs.png",
            filetypes=[("PNG image", "*.png")],
        )
        if not path:
            return
    ...
    def export_csv(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile="chassis_analysis_v2_results.csv",
    ...
    def export_pdf(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile="chassis_analysis_v2_report.pdf",
        ...
        with PdfPages(path) as pdf:
            ...
            summary_text = (
                "TRUCK CHASSIS STRUCTURAL ANALYSIS REPORT V2\n"
