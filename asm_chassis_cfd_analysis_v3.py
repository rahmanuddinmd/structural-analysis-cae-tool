"""
ASM Technologies Ltd - Chassis CFD and structural analysis desktop app V3.

This branded V3 version keeps the V2 chassis calculations and adds a more
professional ASM Technologies Ltd interface for chassis fitting, road/aero load,
material comparison, graph review, and report generation.
"""
# python asm_chassis_cfd_analysis_v3.py
...
from tkinter import BOTH, BooleanVar, Canvas, END, LEFT, PhotoImage, RIGHT, StringVar, Tk, Toplevel, filedialog, messagebox, ttk
...
MATERIALS = {
    "IS 2062 E250 Mild Steel": Material("IS 2062 E250 Mild Steel", 200.0, 250.0, 410.0, 0.30, 7850, "#2f7dc1", "Common Indian chassis/frame fabrication steel."),
    "IS 2062 E350 Structural Steel": Material("IS 2062 E350 Structural Steel", 200.0, 350.0, 490.0, 0.30, 7850, "#1f8a70", "Higher strength structural chassis member material."),
    "ASTM A36 Steel": Material("ASTM A36 Steel", 200.0, 250.0, 400.0, 0.30, 7850, "#4c78a8", "General structural steel comparison baseline."),
    "AISI 1018 Low Carbon Steel": Material("AISI 1018 Low Carbon Steel", 205.0, 370.0, 440.0, 0.29, 7870, "#72b7b2", "Low carbon steel for brackets and welded fittings."),
    "HSS 550 Steel": Material("HSS 550 Steel", 200.0, 550.0, 620.0, 0.30, 7850, "#f58518", "High strength steel for weight-reduced chassis rail study."),
    "HSS 700 Steel": Material("HSS 700 Steel", 200.0, 700.0, 780.0, 0.30, 7850, "#a83a32", "High strength truck frame / heavy load comparison material."),
    "Domex 700 MC": Material("Domex 700 MC", 210.0, 700.0, 750.0, 0.30, 7850, "#9467bd", "Automotive high strength steel used for chassis and transport structures."),
    "Aluminum 6061-T6": Material("Aluminum 6061-T6", 68.9, 276.0, 310.0, 0.33, 2700, "#59a14f", "Lightweight comparison material; deflection usually increases."),
    "CFRP Carbon Fiber": Material("CFRP Carbon Fiber", 70.0, 600.0, 700.0, 0.10, 1600, "#303030", "Advanced lightweight comparison material, not typical for main truck chassis rails."),
    "Titanium Ti-6Al-4V": Material("Titanium Ti-6Al-4V", 113.8, 880.0, 950.0, 0.342, 4430, "#b36b16", "High strength lightweight comparison material, costly for chassis use."),
}

COMPONENTS = {
    "4x2 Truck Main Chassis Rail": Component(
    ...
    "Truck Chassis C-Section Rail": Component(
        "Truck Chassis C-Section Rail",
        5.0,
        0.08,
        0.20,
        "cbeam",
        "Single longitudinal C-section chassis rail. Width means flange width, thickness/height means section height.",
    ),
    "Truck Chassis Box Rail": Component(
    ...
    "Suspension Bracket Zone": Component(
    ...
    "Body Mount Bracket Zone": Component(
...
LOAD_PRESETS = {
    "4x2 Truck Main Chassis Rail": [("Transverse", 9000.0, "Middle"), ("Axial", 3500.0, "Front"), ("Torque", 1200.0, "Rear")],
    "Truck Chassis C-Section Rail": [("Transverse", 12000.0, "Middle"), ("Axial", 5000.0, "Front"), ("Torque", 1800.0, "Rear")],
    "Truck Chassis Box Rail": [("Transverse", 15000.0, "Middle"), ("Axial", 6000.0, "Front"), ("Torque", 2500.0, "Rear")],
    "Cross Member": [("Transverse", 6000.0, "Middle"), ("Axial", 1200.0, "Base"), ("Torque", 700.0, "Bolt Area")],
    "Suspension Bracket Zone": [("Transverse", 18000.0, "Bolt Area"), ("Axial", 4000.0, "Base"), ("Torque", 2200.0, "Bolt Area")],
    "Body Mount Bracket Zone": [("Transverse", 4500.0, "Bolt Area"), ("Axial", 1500.0, "Base"), ("Torque", 600.0, "Bolt Area")],
}
...
APP_TITLE = "Chassis CFD & Structural Analysis"
...
class ChassisApp:
    def __init__(self, root: Tk):
        ...
        self.component_name = StringVar(value="4x2 Truck Main Chassis Rail")
        self.material_name = StringVar(value="IS 2062 E350 Structural Steel")
        self.compare_name = StringVar(value="HSS 700 Steel")
        self.boundary = StringVar(value="Simply Supported")
        self.status_text = StringVar(value="Select chassis specification, edit loads, then run analysis.")
        self.cad_loaded_var = StringVar(value="No CAD part loaded")
        self.cad_detail_var = StringVar(value="Upload a STEP/STL/CATPart/mesh file to auto-detect the chassis preset.")
    ...
    def configure_window_size(self):
        ...
        width = min(1280, max(980, int(screen_w * 0.92)))
        height = min(820, max(640, int(screen_h * 0.86)))
        min_w = min(980, max(860, screen_w - 120))
        min_h = min(640, max(560, screen_h - 160))
        pos_x = max((screen_w - width) // 2, 0)
        pos_y = max((screen_h - height) // 2, 0)
        self.root.geometry(f"{width}x{height}+{pos_x}+{pos_y}")
        self.root.minsize(min_w, min_h)
        self.compact_ui = screen_w < 1400 or screen_h < 850

    def build_ui(self):
        style = ttk.Style()
        style.theme_use("clam")
        title_size = 12 if self.compact_ui else 14
        metric_size = 13 if self.compact_ui else 16
        style.configure(".", font=("Segoe UI", 9), background="#ffffff")
        style.configure("TFrame", background="#ffffff")
        style.configure("Brand.TFrame", background=ASM_BLUE)
        style.configure("Panel.TLabelframe", background=ASM_PANEL, bordercolor="#c9d6e6", relief="solid")
        style.configure("Panel.TLabelframe.Label", background=ASM_PANEL, foreground=ASM_BLUE_DARK, font=("Segoe UI", 9, "bold"))
        style.configure("Title.TLabel", font=("Segoe UI", title_size, "bold"), foreground=ASM_BLUE_DARK, background="#ffffff")
        style.configure("BrandTitle.TLabel", font=("Segoe UI", title_size + 3, "bold"), foreground="#ffffff", background=ASM_BLUE)
        style.configure("BrandSub.TLabel", foreground="#eaf2fb", background=ASM_BLUE)
        style.configure("Sub.TLabel", foreground="#4d5b6b", background="#ffffff")
        style.configure("Metric.TLabel", font=("Segoe UI", metric_size, "bold"), foreground=ASM_BLUE_DARK, background="#ffffff")
        style.configure("Loaded.TLabel", font=("Segoe UI", metric_size, "bold"), foreground="#147a35", background="#eaf7ee")
        style.configure("LoadedDetail.TLabel", foreground="#145a2a", background="#eaf7ee")
        style.configure("Loaded.TFrame", background="#eaf7ee")
        style.configure("Warn.TLabel", foreground="#a83a32", background="#ffffff")
        style.configure("TButton", padding=(10, 5))
        style.map("TButton", background=[("active", "#e7eef7")])
        style.configure("TNotebook", background="#ffffff", borderwidth=0)
        style.configure("TNotebook.Tab", padding=(14, 7), foreground=ASM_BLUE_DARK)
        ...
        header = ttk.Frame(container, style="Brand.TFrame", padding=(14, 10))
        ...
        ttk.Label(title_block, text=COMPANY_NAME, style="BrandTitle.TLabel").pack(anchor="w")
        ttk.Label(
            title_block,
            text=f"{APP_TITLE} {APP_VERSION} | Chassis fitting, road/aero loading, material comparison, graphs, and PDF report",
            style="BrandSub.TLabel",
        ).pack(anchor="w", pady=(2, 0))
        ttk.Label(header, text="engineering innovation", style="BrandSub.TLabel").pack(side=RIGHT, padx=(10, 0))
    ...
    def load_logo_image(self):
        ...
        if image.width() > max_width:
            factor = max(1, math.ceil(image.width() / max_width))
    ...
    def build_inputs(self, parent):
        ...
        option_tabs.add(load_tab, text="Load Case")
        option_tabs.add(road_tab, text="Road/Air Load")

        file_box = ttk.LabelFrame(setup_tab, text="Chassis / CAD Input", padding=10, style="Panel.TLabelframe")
        file_box.pack(fill="x", pady=(0, 8))
        ttk.Button(file_box, textvariable=self.cad_button_var, command=self.upload_part).pack(fill="x")
        self.loaded_panel = ttk.Frame(file_box)
        loaded_panel = self.loaded_panel
        loaded_panel.pack(fill="x", pady=(8, 0))
        ttk.Label(loaded_panel, text="CAD Status", style="Title.TLabel").pack(anchor="w")
        self.cad_status_label = ttk.Label(loaded_panel, textvariable=self.cad_loaded_var, style="Metric.TLabel", wraplength=310)
        self.cad_status_label.pack(anchor="w", pady=(2, 0))
        self.cad_detail_label = ttk.Label(loaded_panel, textvariable=self.cad_detail_var, style="Sub.TLabel", wraplength=310)
        self.cad_detail_label.pack(anchor="w", pady=(2, 0))
        ttk.Label(
            file_box,
            text="Supported for preset selection: file names containing chassis, frame, rail, cross, member, suspension, bracket.",
            wraplength=300,
            style="Sub.TLabel",
        ).pack(anchor="w", pady=(6, 0))

        comp_box = ttk.LabelFrame(setup_tab, text="Chassis Specification Table", padding=10, style="Panel.TLabelframe")
        comp_box.pack(fill="x", pady=(0, 8))
        ttk.Label(comp_box, text="Chassis Part").grid(row=0, column=0, sticky="w")
        ...
        for row, (label, var, unit) in enumerate(
            [
                ...
                ("Flange / width", self.width_var, "m"),
                ("Section height", self.thickness_var, "m"),
        ...
        mat_box = ttk.LabelFrame(setup_tab, text="Material Used For Chassis", padding=10, style="Panel.TLabelframe")
        ...
        load_box = ttk.LabelFrame(load_tab, text="Load Input Table", padding=10, style="Panel.TLabelframe")
        ...
        bc_box = ttk.LabelFrame(load_tab, text="Boundary Condition", padding=10, style="Panel.TLabelframe")
        bc_box.pack(fill="x", pady=(0, 8))
        ttk.Combobox(bc_box, textvariable=self.boundary, values=BOUNDARIES, state="readonly").pack(fill="x")

        cfd_box = ttk.LabelFrame(road_tab, text="V3 Chassis CFD / Road-Air Load", padding=10, style="Panel.TLabelframe")
        cfd_box.pack(fill="x", pady=(0, 8))
        ttk.Checkbutton(cfd_box, text="Use aerodynamic road load in analysis", variable=self.cfd_enabled).grid(
        ...
        ttk.Button(cfd_box, text="Calculate and Add Road/Air Load", command=self.apply_cfd_load).grid(
    ...
    def build_outputs(self, parent):
        ...
        figure_size = (8.6, 5.7) if self.compact_ui else (9.6, 6.4)
        ...
        formula_box = ttk.LabelFrame(parent, text="Strength of Materials Backend Formulas", padding=10, style="Panel.TLabelframe")
    ...
    def detect_chassis_part_from_filename(self, filename):
        ...
        if "suspension" in name or "bracket" in name or "mount" in name:
            return "Suspension Bracket Zone", "Detected suspension/bracket/mount keyword"
        if "box" in name or "boxed" in name:
            return "Truck Chassis Box Rail", "Detected box/boxed rail keyword"
        if "4x2" in name or "four" in name or "front" in name or "rear" in name or "main" in name:
            return "4x2 Truck Main Chassis Rail", "Detected 4x2/front/rear/main chassis keyword"
        if "rail" in name or "chassis" in name or "frame" in name or "beam" in name:
            return "Truck Chassis C-Section Rail", "Detected chassis/frame/rail keyword"
        return self.component_name.get(), "No filename keyword matched; current chassis selection kept"
    ...
    def upload_part(self):
        ...
        if not path:
            return
        cad_path = Path(path)
        detected_part, reason = self.detect_chassis_part_from_filename(cad_path.name)
        ...
        self.loaded_panel.configure(style="Loaded.TFrame")
        self.cad_status_label.configure(style="Loaded.TLabel")
        self.cad_detail_label.configure(style="LoadedDetail.TLabel")
        self.status_text.set(f"CAD part loaded: {cad_path.name} -> {detected_part}")
        messagebox.showinfo("CAD part loaded", f"Loaded: {cad_path.name}\nDetected: {detected_part}")
    ...
    def open_graph_window(self):
        if not self.current_result:
            messagebox.showwarning("No results", "Run analysis before opening graphs.")
            return

        graph_window = Toplevel(self.root)
        graph_window.title(f"{COMPANY_NAME} - Chassis CFD Analysis Graphs {APP_VERSION}")
        graph_window.geometry("1500x850+120+80")
        graph_window.minsize(1100, 700)

        toolbar = ttk.Frame(graph_window, padding=8)
        toolbar.pack(fill="x")
        ttk.Label(toolbar, text=f"{COMPANY_NAME} | Graph View - Stress, Strain, Deformation, and FOS", style="Title.TLabel").pack(side=LEFT)
        ttk.Button(toolbar, text="Save PNG", command=lambda: self.save_figure_from_window(graph_fig)).pack(side=RIGHT)
        ...
        graph_canvas.get_tk_widget().pack(fill=BOTH, expand=True, padx=8, pady=(0, 8))
        ...
        self.draw_plots(MATERIALS[self.material_name.get()], self.current_result)
        self.axes = old_axes
    ...
    def save_figure_from_window(self, figure):
        path = filedialog.asksaveasfilename(
            title="Save ASM V3 graph window image",
            defaultextension=".png",
            initialfile="asm_chassis_cfd_analysis_v3_graph_window.png",
            filetypes=[("PNG image", "*.png")],
        )
        if path:
            figure.savefig(path, dpi=200)
            self.status_text.set(f"Saved graph window image: {path}")
    ...
    def export_graphs(self):
        if not self.current_result:
            ...
            return
        path = filedialog.asksaveasfilename(
            title="Save ASM V3 non-editable graph image",
            defaultextension=".png",
            initialfile="asm_chassis_cfd_analysis_v3_graphs.png",
            filetypes=[("PNG image", "*.png")],
        )
        if not path:
            return
    ...
    def export_csv(self):
        if not self.current_result:
            ...
            return
        path = filedialog.asksaveasfilename(
            title="Save ASM V3 result table",
            defaultextension=".csv",
            initialfile="asm_chassis_cfd_analysis_v3_results.csv",
    ...
    def export_pdf(self):
        if not self.current_result:
            ...
            return
        path = filedialog.asksaveasfilename(
            title="Save ASM V3 PDF report",
            defaultextension=".pdf",
            initialfile="asm_chassis_cfd_analysis_v3_report.pdf",
        ...
        with PdfPages(path) as pdf:
            ...
            summary_text = (
                f"{COMPANY_NAME.upper()} - CHASSIS CFD & STRUCTURAL ANALYSIS REPORT V3\n"
